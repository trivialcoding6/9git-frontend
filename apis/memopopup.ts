...
export const fetchMemos = async (
  ...
  const res = await fetch(
    `${baseUrl}/api/v1/users/${userId}/categories/${categoryId}/${year}/${month}/memos/`
  );
...
export const createMemo = async (memo: Omit<Memo, 'id'>, userId: number): Promise<Memo> => {
  const response = await fetch(`${baseUrl}/api/v1/users/${userId}/memos`, {
    method: 'POST',
    headers: {
...
export const updateMemo = async (
  ...
  const response = await fetch(`${baseUrl}/api/v1/users/${userId}/memos/${id}`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
    },
